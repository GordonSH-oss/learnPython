...
const downloadLink = getElement("download");
...
async function processRecording() {
    try {
        ...
        const sourceBuffer = await audioContext.decodeAudioData(await recordedBlob.arrayBuffer());
...
async function filterAudio(input) {
    const offlineContext = new OfflineAudioContext(input.numberOfChannels, input.length, input.sampleRate);
...
function normalizeAudio(buffer, targetPeak) {
    let peak = 0;
